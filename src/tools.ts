...
export function buildTools(): ToolDef[] {
  ...
  function agentView(handle: string, a: { name: string; endpoint_url?: string; last_seen?: string }, kind: "own" | "peer") {
    ...
    return { handle, person, agent: a.name, kind, can_answer_now: reachable, delivery: reachable ? "sync" : "inbox (inline while its agent runner is listening)", last_seen: a.last_seen ?? null };
  }
